...
@Component
public class CustomerRejectedResponseListener {

    private static final Logger log = LoggerFactory.getLogger(CustomerRejectedResponseListener.class);

    private final ReservationSagaOrchestrator orchestrator;
    private final ObjectMapper objectMapper;

    public CustomerRejectedResponseListener(ReservationSagaOrchestrator orchestrator,
                                            ObjectMapper objectMapper) {
        this.orchestrator = orchestrator;
        this.objectMapper = objectMapper;
    }

    @RabbitListener(queues = "customer.rejected.queue")
    public void handle(Message message) {
        ...
        log.warn("Received customer rejected response for reservation {}: {}", reservationId, failureMessages);
        orchestrator.handleStepFailure(reservationId, "CUSTOMER_VALIDATION", failureMessages);
    }
    ...
    private JsonNode parseMessage(Message message) {
        try {
            ...
            throw new IllegalStateException("Failed to parse customer rejected response", e);
        }
    }
}
