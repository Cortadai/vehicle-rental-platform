...
@Component
public class FleetRejectedResponseListener {

    private static final Logger log = LoggerFactory.getLogger(FleetRejectedResponseListener.class);

    private final ReservationSagaOrchestrator orchestrator;
    private final ObjectMapper objectMapper;

    public FleetRejectedResponseListener(ReservationSagaOrchestrator orchestrator,
                                         ObjectMapper objectMapper) {
        this.orchestrator = orchestrator;
        this.objectMapper = objectMapper;
    }

    @RabbitListener(queues = "fleet.rejected.queue")
    public void handle(Message message) {
        ...
        log.warn("Received fleet rejected response for reservation {}: {}", reservationId, failureMessages);
        orchestrator.handleStepFailure(reservationId, "FLEET_CONFIRMATION", failureMessages);
    }
    ...
    private JsonNode parseMessage(Message message) {
        try {
            ...
            throw new IllegalStateException("Failed to parse fleet rejected response", e);
        }
    }
}
