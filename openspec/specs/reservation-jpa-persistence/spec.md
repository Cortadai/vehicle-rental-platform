...
- **AND** `failureMessages` SHALL be serialized to JSON via `objectMapper.writeValueAsString()` (or null if empty)
...
- **AND** it SHALL deserialize `failureMessages` from JSON via `objectMapper.readValue()` with `TypeReference<List<String>>` (or empty list if null/blank)
