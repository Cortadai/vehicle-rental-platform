...
- **WHEN** FleetServiceApplication annotations are inspected
...
BeanConfiguration SHALL be a `@Configuration` class that manually creates beans for classes that have no Spring annotations (domain and application layer classes). It SHALL register `ConfirmFleetAvailabilityUseCase` and `ReleaseFleetReservationUseCase` in addition to the 5 existing use case beans.
...
- **WHEN** the application context is loaded
- **THEN** beans for `RegisterVehicleUseCase`, `GetVehicleUseCase`, `SendToMaintenanceUseCase`, `ActivateVehicleUseCase`, `RetireVehicleUseCase`, `ConfirmFleetAvailabilityUseCase`, and `ReleaseFleetReservationUseCase` SHALL be available
...
- **WHEN** `FleetServiceApplicationIT`, `VehicleControllerIT`, or `VehicleRepositoryAdapterIT` is executed
...
The fleet-container POM SHALL include test dependencies for `org.testcontainers:rabbitmq` and `org.awaitility:awaitility`.
...
OutboxAtomicityIT SHALL verify that fleet entity persistence and outbox event insertion occur atomically within the same transaction.

#### Scenario: Vehicle registration produces outbox event atomically

- **WHEN** a vehicle is registered via the application service
- **THEN** both the vehicle entity and the corresponding outbox event SHALL be persisted
